...
class Eac_Custom_Attributes {
	...
	private function parse_custom_attributes( $attributes_string, $delimiter = ',' ) {
		...
		/** Boucle sur les Clé|Valeur */
		foreach ( $attributes as $attribute ) {
			$attr_key_value = explode( '|', $attribute );

			$attr_key = mb_strtolower( $attr_key_value[0], 'UTF-8' );
			...
			/**
			 * On saute les événements Javascript, la clé href because injection XSS et les styles
			 */
			if ( 'href' === $attr_key || 'on' === substr( $attr_key, 0, 2 ) || 'style' === $attr_key ) {
				continue;
			}
	 ...
	 *
	 * Retourne la liste des attributs interdits utilisés dans les section, colonne, widget et container Elementor
	 */
	private function get_black_list_attributes() {
		...
		if ( null === $black_list ) {
			 ...
			 *
			 * Protéger Elementor des attributs qui ne peuvent être utilisés pour éviter un crash
			 *
