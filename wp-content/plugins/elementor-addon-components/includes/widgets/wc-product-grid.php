...
class WC_Product_Grid_Widget extends Widget_Base {
	...
	use \EACCustomWidgets\Includes\Widgets\Traits\Badge_Promo_Trait;
	...
	public function __construct( $data = array(), $args = null ) {
		...
		if ( Eac_Config_Elements::is_a_block_theme() ) {
			...
			wp_register_script( 'eac-post-grid', EAC_Plugin::instance()->get_script_url( 'assets/js/elementor/eac-post-grid' ), array( 'jquery', 'elementor-frontend', 'isotope', 'eac-infinite-scroll', 'swiper', 'eac-imagesloaded' ), '1.9.8', true );
		}
		...
		wp_enqueue_style( 'eac-product', EAC_Plugin::instance()->get_style_url( 'assets/css/wc-product' ), array( 'eac-post-grid' ), EAC_PLUGIN_VERSION );
	...
	public function get_script_depends() {
		if ( Eac_Config_Elements::is_a_block_theme() ) {
			return array( 'isotope', 'eac-imagesloaded', 'eac-infinite-scroll', 'swiper', 'eac-fit-rows' );
		} else {
		...
		if ( $the_query->have_posts() ) {
			...
			<div <?php $this->print_render_attribute_string( 'posts_wrapper' ); ?>>
				...
				while ( $the_query->have_posts() ) {
					 ...
					 * Ajout des attributs width et height pour le lazyload
					 */
					if ( ! $has_swiper ) {
					...
					$post_class = $article_class . ' ' . implode( ' ', get_post_class( '', $product_id ) );
					?>
					<article id="<?php echo 'post-' . esc_attr( $product_id ); ?>" class="<?php echo esc_attr( $post_class ); ?>">
						<div class='al-post__inner-wrapper'>
							...
							<?php if ( $has_stock_badge && ! $is_a_catalog ) : ?>
								<span class='badge-stock'><?php echo esc_html( sanitize_text_field( $settings['stock_text'] ) ); ?></span>
							<?php endif; ?>
							<?php if ( ! $has_expired ) : ?>
								<span class='badge-new'><?php echo esc_html( sanitize_text_field( $settings['new_text'] ) ); ?></span>
							<?php endif; ?>
							...
							<div <?php $this->print_render_attribute_string( 'content_wrapper' ); ?>>
									...
									<div class='al-post__image-wrapper'>
										...
										if ( $attachment ) {
											...
											<?php if ( $has_image_link ) : ?>
												<a class='eac-accessible-link' href="<?php echo esc_url( $product_url ); ?>" aria-label="<?php echo esc_html__( 'Voir la page produit', 'eac-components' ) . ' ' . esc_html( $product_title ); ?>">
											<?php endif; ?>
								...
								<?php endif; ?>
									...
									<?php if ( $has_title ) : ?>
										<!-- Affiche les IDs -->
										<?php if ( $has_id && $has_title_link ) : ?>
											<a class='eac-accessible-link' href="<?php echo esc_url( $product_url ); ?>">
										...
										<div class='buttons-wrapper'>
											<?php if ( $has_readmore ) : ?>
												<span class='button__readmore-wrapper'>
													<a href="<?php echo esc_url( $product_url ); ?>" class='button-readmore' role='button' aria-label="<?php echo esc_html( sanitize_text_field( $settings['button_more_label'] ) ) . ' ' . esc_html( $product_title ); ?>">
														<?php
											...
											<?php if ( $has_date ) : ?>
												<span class='al-post__meta-date'>
													<?php if ( 'modified' === $settings['al_article_orderby'] ) : ?>
														<i class='fas fa-calendar' aria-hidden='true'></i><?php echo esc_html( $product->get_date_modified()->date( get_option( 'date_format' ) ) ); ?>
													<?php else : ?>
														<i class='fas fa-calendar' aria-hidden='true'></i><?php echo esc_html( $product->get_date_created()->date( get_option( 'date_format' ) ) ); ?>
													<?php endif; ?>
												</span>
											<?php endif; ?>
										</div>
			...
			<?php if ( $has_pagging && $the_query->post_count < $the_query->found_posts ) :
				...
				$button_text = '<button ' . $this->get_render_attribute_string( 'read_button' ) . '>' . esc_html__( 'Plus de produits', 'eac-components' ) . ' <span class="al-more-button-paged">' . $the_query->post_count . '/' . $the_query->found_posts . '</span></button>';
				?>
