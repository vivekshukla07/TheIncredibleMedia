	...
	var EacAddonsPinterestRss = {
		...
		widgetPinterestRss: function widgetPinterestRss($scope) {
			var $targetInstance = $scope.find('.eac-pin-galerie'),
				$targetSelect = $scope.find('.select__options-items'),
			...
			$targetButton.on('click touch', function(e) {
				...
				instanceAjax = new EacReadTheFeed($('.select__options-items option:selected', $targetInstance).val().replace(/\s+/g, ''), targetNonce, settings.data_id);
				$targetLoader.show();
			});
			...
			$(document).on('ajaxComplete', function(event, xhr, ajaxSettings) {
				if(instanceAjax != null && ajaxSettings.ajaxOptions && ajaxSettings.ajaxOptions === instanceAjax.getOptions()) { // Le même random number généré lors de la création de l'objet Ajax
					...
					$targetHeader.append('<span><a href="' + Profile.headLink + '"><h2>' + Profile.headTitle + '</h2></a></span>');
					...
					$.each(Items, function(index, item) {
						...
						var titre = '<div class="pin-galerie__item-link-post"><a href="' + item.lien + '"><h2 class="pin-galerie__item-titre">' + item.title + '</h2></a></div>';
						...
						if(settings.data_date) {
							var dateUpdate =  '<div class="pin-galerie__item-date"><i class="fas fa-calendar" aria-hidden="true"></i>' + new Date(item.update).toLocaleDateString() + '</div>';
							var Auteur =  '<div class="pin-galerie__item-auteur"><i class="fas fa-user" aria-hidden="true"></i>' + item.author + '</div>';
