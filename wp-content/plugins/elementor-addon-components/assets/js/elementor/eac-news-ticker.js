	...
	var EacAddonsNewsTicker = {
		...
		widgetNewsTicker: function widgetNewsTicker($scope) {
			var $targetInstance = $scope.find('.eac-news-ticker'),
				...
				targetNonce = $targetInstance.find('#news_nonce').val(),
			...
			$targetWrapperControlPlay.on('keydown click', function (evt) {
				...
				$targetWrapperControlPause.focus();
			...
			$targetWrapperControlPause.on('keydown click', function (evt) {
				const id = evt.code || evt.key || 0;
				if ('keydown' === evt.type && ('Enter' !== id && 'Space' !== id)) {
					return;
				}
				...
				$targetWrapperControlPlay.focus();
			 ...
			 */
			$targetWrapperControlLeft.on('keydown click', function (evt) {
				const id = evt.code || evt.key || 0;
				if ('keydown' === evt.type && ('Enter' !== id && 'Space' !== id)) {
					return;
				}
			 ...
			 * Reinitialise les différents css
			 */
			$targetWrapperControlRight.on('keydown click', function (evt) {
				const id = evt.code || evt.key || 0;
				if ('keydown' === evt.type && ('Enter' !== id && 'Space' !== id)) {
					return;
				}
		...
		setNewsTickerElements: function setNewsTickerElements(itemFeed, settings, nonce, $targetTitle, $targetContent) {
			...
			instanceAjax  = new EacReadTheFeed(currentItemUrl.replace(/\s+/g, ''), nonce, settings.data_id);
